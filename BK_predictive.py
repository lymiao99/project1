...
def sensor_stream():
    ...
    try:
        if last_udi is None:
            query = text(f"SELECT * FROM {TABLE_NAME} ORDER BY udi DESC LIMIT 1")
            params = {}
        else:
            query = text(f"SELECT * FROM {TABLE_NAME} WHERE udi > :last_udi ORDER BY udi ASC LIMIT 1")
        ...
        with engine.connect() as conn:
            ...
            if row is None:
                row = conn.execute(
                    text(f"SELECT * FROM {TABLE_NAME} ORDER BY udi DESC LIMIT 1")
                ).fetchone()
...
def predict_route():
    ...
    try:
        ...
        recommendation = "Schedule maintenance immediately" if prediction == 1 else "No immediate action required"
...
def api_stats():
    ...
    try:
        ...
        trend_df = pd.read_sql(trend_query, engine)
        trend_data = trend_df['rotational_speed_rpm'].iloc[::-1].tolist() # ??????
